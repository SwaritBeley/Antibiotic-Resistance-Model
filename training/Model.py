...
# df = pd.read_csv(filepath_Training)

# x_df = df.drop(columns=['bacterium', 'antibiotic', 'n_isolates', 'observed_resistance'], axis=1)
# y_df = df['observed_resistance']

# model = sk.RandomForestRegressor(n_estimators=100, random_state=42)
# model.fit(x_df, y_df)
...
for train_index, test_index in loo.split(X):
    ...
    model = RandomForestRegressor(n_estimators=500, random_state=42, max_features=0.75)
